...
The objective below combines contrail ATR20 (in K/km, multiplied by airspeed to give K/s) and CO2 (fuel-flow kg/s multiplied by 7.03e-15 K/kg to give K/s). The total is tiny — somewhere around 10⁻¹² K/s per collocation segment.

IPOPT's default convergence tolerances (`tol`, `acceptable_tol`) are calibrated for objectives on the order of 1. When the physical objective is at 10⁻¹², the tolerances are trivially satisfied at any feasible point: the solver reports `Solve_Succeeded` in a handful of iterations, but the optimum it returns is barely distinguishable from the initial guess. All scenarios below would collapse to the same trajectory.

`auto_rescale_objective=True` evaluates the symbolic objective at the initial guess, divides the whole objective by that value, and multiplies back into `objective_value` so callers still see the physical number. IPOPT then works against an `O(1)` objective where its tolerances mean what they say. Use this whenever the natural magnitude of the objective is far from 1 — climate-metric examples are the canonical case.
:::

:::{.callout-tip}
## Tip: precompute the interpolant for contrail + CO2 solves

For the `Cruise` example below, `interpolant_from_dataframe(df_cost)` is quick to build. For `CompleteFlight` solves over large, high-resolution contrail grids it can take several minutes, and linear interpolation additionally has discontinuous derivatives at every grid-cell boundary — IPOPT's line search can oscillate on the resulting blended objective.

The recommended path is to precompute a cubic B-spline interpolant once and cache it to disk:
...
The first call builds the B-spline and writes the result to `cache/contrail.casadi`; subsequent calls load the cache in under a second. The same cache can be produced from the command line with `opentop gengrid` — see the [CLI chapter](cli.qmd) for details.
:::
...
optimizer.setup(debug=False, max_iter=2000)
...
def plot_flight(results, df_cost, scenarios=None):
    ...
    for scenario, flight in results.items():
        ax.plot(
            ...
            transform=ccrs.PlateCarree(),
            lw=2,
            label=f"{scenario} | fuel: {int(flight.fuel_cost.sum())} kg",
        )
...
accf_fuel = results["accf_optimal"].fuel_cost.sum()
fuel_fuel = results["fuel_optimal"].fuel_cost.sum()
