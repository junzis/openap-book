...
:::{.callout-tip}
## Tip: cache the interpolant for larger grids

For the toy 40×40×32 grid in this chapter, `interpolant_from_dataframe` is quick. For a realistic regional weather or contrail grid, building a cubic B-spline interpolant can take several minutes, and any `CompleteFlight` solve that queries it benefits from the smoother derivatives that a B-spline provides. Use `cached_interpolant_from_dataframe` to build the interpolant once and cache it to disk:

```python
from opentop.tools import cached_interpolant_from_dataframe

interpolant = cached_interpolant_from_dataframe(
    df_cost_3d, "cache/grid.casadi", shape="bspline"
)
```

See the [CLI chapter](cli.qmd) for `opentop gengrid`, which produces the same cache from a command line.
:::
...
def objective(x, u, dt, **kwargs):
    ...
    grid_cost = optimizer.obj_grid_cost(x, u, dt, n_dim=3, time_dependent=True, **kwargs)
