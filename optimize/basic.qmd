...
```python
optimizer = top.CompleteFlight(actype, origin, destination, m0, engine="CFM56-5B4")
```
...
```python
flight["fuel_cost"].sum()  # total fuel in kg
```

The optimizer object exposes the solver state after the solve:

```python
optimizer.objective_value   # final objective value
optimizer.solver            # ca.OptiSol
optimizer.solver.stats()    # iter_count, success, return_status, ...
```
